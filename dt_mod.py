...
# This basically checks foor presence of function and if it is then returns the list of names of function in the code
...
def generate_ast(code):
    ...
    #Retriveing AST corresponding to the code if present in cache
    if code in ast_cache:
        return ast_cache[code]
    
    #ast_cache dictionary basically has key as the code and value as the tu.cursor corresponding to the code
    ...
    #AST generated can be traversed with the help of cursor of the clang library
    return tu.cursor

def create_tree(cursor):
    node = Node(str(cursor.kind))
...
def compare_functions(file_name1, func1, file_name2, func2):
    ...
    tree1 = create_tree(generate_ast(code1))
    tree2 = create_tree(generate_ast(code2))
...
def extract_function_code(file_name, function_name):
    ...
    brace_count = 0
    index = func_start + len(function_name)
